...
interface RefrigerantFormProps {
    ...
    totalRefrigerantCharge: number
    handleAdditionalRefrigerantCharge: (value: number) => void;
    handlePrechargedRefrigerantCharge: (value: number) => void;
    totalVolume: number;
    handleArea: (value: number) => void;
                ...
                <div className="flex flex-col items-center">
                    {
                        condenserList.map((_condenser, condenserIndex) => (
                            <div className="flex flex-row  pt-6" key={condenserIndex}>
                                ...
                                <div className="flex flex-col items-center font-mono gap-2 text-gray-600 pl-4">
                                    ...
                                    <div><Input placeholder="Additional Refrigerant" className="text-center w-32" type="number" id="AR" onChange={(e) => handleAdditionalRefrigerantCharge(parseFloat(e.target.value))} /> </div>
                                    <div><Input placeholder="Precharged Refrigerant" className="text-center w-32" type="number" id="PR" onChange={(e) => handlePrechargedRefrigerantCharge(parseFloat(e.target.value))} /> </div>
                                    <div className="text-gray-400"> Result: {totalRefrigerantCharge} kg </div>

                                </div>
                                <div className="flex flex-col items-center font-mono gap-1  text-gray-600 pl-4">
                                    <div> Area Served </div>
                                    {fcuList[condenserIndex].map((_fcu, fcuIndex) => (
                                        <div key={fcuIndex}><Input placeholder="Enter Area Served" className="text-center w-32" /> </div>
                                    ))}
                                </div>
                                <div className="flex flex-col items-center font-mono gap-1  text-gray-600 pl-4">
                                    <div> Area m<sup>2</sup></div>
                                    {fcuList[condenserIndex].map((_fcu, fcuIndex) => (
                                        <div key={fcuIndex}><Input placeholder="Enter Area" className="text-center w-32" type="number" onChange={(e) => handleArea(parseFloat(e.target.value))} /> </div>
                                    ))}
                                </div>
                                <div className="flex flex-col items-center font-mono gap-1  text-gray-600 pl-4">
                                    <div> Height m</div>
                                    {fcuList[condenserIndex].map((_fcu, fcuIndex) => (
                                        <div key={fcuIndex}><Input placeholder="Enter height" className="text-center w-32" type="number" onChange={(e) => handleHeight(parseFloat(e.target.value))} /> </div>
                                    ))}
                                </div>
                                <div className="flex flex-col items-center font-mono gap-2  text-gray-600 pl-4">
                                    <div> Volume m<sup>3</sup></div>
                                    {fcuList[condenserIndex].map((_fcu, fcuIndex) => (
                                        <div key={fcuIndex} className="text-base text-gray-400">{totalVolume} m<sup>3</sup></div>
                                    ))}
                                </div>
                                <div className="flex flex-col items-center font-mono gap-2 text-gray-600 pl-4">
