    ...
    return (
        <div className='flex pt-2 justify-center'>
            ...
            <div className='relative' onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
                <div className={`group-hover:block absolute top-[45%] -translate-x-[600px] translate-y-[-50%] left-5 text-2xl rounded-full p-2 bg-black/20 text-white cursor-pointer ${!isHovered && 'hidden'}`}>
                    <BsChevronCompactLeft onClick={prevSlide} size={30} />
                </div>
                <div className={`group-hover:block absolute top-[45%] -translate-x-[-600px] translate-y-[-50%] right-5 text-2xl rounded-full p-2 bg-black/20 text-white cursor-pointer ${!isHovered && 'hidden'}`}>
